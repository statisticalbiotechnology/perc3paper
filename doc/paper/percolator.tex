...
proteomics datasets with Percolator 3.0}
...
%{\em FIXME: I'm not sure this is the right article to cite for this.
%--Matthew. This is apt. Particularly as you give more modern cites
%below. --Lukas}
...
PSMs.  The Kim data set might not be as representative for other types
of studies that have greater heterogeneity, but it seems likely that
the idea will hold as long as the selected subsets are not too small.
...
%should report this value somewhere. --Bill. I agree --Lukas} 
...
overfitting when the strategy is carried out on large datasets. 
%{\em FIXME: I don't
%  understand why this is necessary?  Why can't you downsample first
%  and then do all the cross-validation the same as usual? --Bill This is just to be able to keep the current training procedure as is. We could have made asymmetric train and test parts of each cross validation set, or possibly other arrangements, but this just turned out to play better with the current version of the code. --Lukas}
...
%for smaller studies? --Bill. It works for smaller scale studies, but we do not provide evidence for that. Is that right Matthew? --Lukas}
