...
\caption{\label{fig:unique-accuracy}\textbf{Using only protein-unique
  ...
  {\em FIXME: What peptide-level FDR was used? --Bill. I do not
    ...
    suggesting changing the plot.  --Bill. Matthew never used any
    peptide-level threshold. The methods all produce one consistent
    list of proteins each; lists that do not change when we adjust the
    FDR threshold. --Lukas} All four methods produce accurate FDR
  estimates. (B) A logarithmic plot of the region $[0.001, 0.1]$ with
  the same axes as in (A). Please note that in contrast to the
  procedure used in Figure \ref{fig:shared-accuracy}, we did not apply
  any peptide level FDR as all protein grouping was made before data
  was observed. }
\end{center}
\end{figure}
...
two-peptide rule and Fisher's method identify far less protein groups.
...
We plotted the average number of
...
sets, we looked at the number of inferred protein groups for
downsampled subsets of the Kim data set
(Figure~\ref{fig:power-downsample}). For small data sets, all methods
except the two-peptide rule perform about equally. However, as the
data sets get larger, the best-scoring peptide approach starts to show
its advantage. Overall, we concluded, in agreement with Savitski {\em
et al.} \cite{savitski2015scalable}, that the best-scoring peptide
...
long as the selected subsets are not too small and contain a
sufficient number of positive training examples.  {\em FIXME: I think
  ...
  somewhere, and I am not seeing it? --Bill. We spelled out the number
  of PSMs in the text above. Did you mean something different? What
  did you mean with first round of training, is it the number of PSMs
  in the initial search direction? --Lukas} In particular, our
...
strategy is carried out on large data sets. This strategy was adopted
for simplicity of implementation. In a future version of the code, we
will likely implement a scheme where we downsample each cross
validation bin individually.
...
{\em FIXME: I don't
  ...
  simplicity of implementation, but ...'' -- Bill. Maybe that I do not
  understand with what you mean with ``the same as usual''. I do not
  think there is a default strategy for how to downsample in this
  case. One other option would be to downsample each cross validation
  bin separately. Is that what you had in mind? --Lukas}
