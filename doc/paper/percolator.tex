...
\caption{\label{fig:unique-accuracy}\textbf{Using only protein-unique
  ...
  {\em FIXME: What peptide-level FDR was used? --Bill. I do not
    ...
    suggesting changing the plot.  --Bill. No, Matthew never used any
    peptide-level treshold. The methods all produce one consistent
    list of proteins each. Lists that do not change when we adjust the
    FDR treshold. --Lukas}  All four methods produce accurate FDR
...
strategy is carried out on large data sets.  {\em FIXME: I don't
  ...
  understanbd with what yuou mean with ``the same as usual''. I do not
  think there is a default startegy for how to downsample in this
